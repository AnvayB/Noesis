...
export const explainBackAiAbstraction: CurriculumModule = {
    ...
    sections: [
      {
        ...
        body:
          ...
          "ExplainBackInput carries more than just the raw text: knownFields (so a new concept gets filed under an existing field when it fits, rather than fragmenting the map's regions), sourceExcerpt (the article's actual extracted text when the session came from a link, via lib/extract.ts — capped before being sent), marks (whatever the learner jotted while reading or watching), and sourceTitle. ExplainBackAnalysis correspondingly returns more than a verdict: relatedKnown (prior concepts the material plainly relates to, whether or not the learner said so — these steer the Mindscape's growth but, unlike connectionsMade, never fuse into a cord, since only a connection the learner stated themselves earns that), a one-sentence gist in the learner's own terms, a 1-to-5 level on an explicit ladder (1 'heard of it', 5 'could teach it'), and a concrete nextStep to reach the level above.\n\n" +
  ...
  levels: {
    explain: {
      ...
      groundTruth:
        "The abstraction (types.ts interfaces + schemas.ts zod mirrors + a provider-selecting factory in index.ts) means application code only ever calls ai.analyzeExplainBack(...) and never imports an OpenAI (or any vendor) type — swapping or adding a provider touches only lib/ai/. The LLM's responsibility is strictly the ExplainBackAnalysis shape: it judges depth/clarity, flags omissions/misconceptions, proposes concept connections and a field for each, and — given the source article's actual text when there is one — judges omissions against what the material really said rather than general knowledge. It does NOT decide how that gets stored, does NOT dedupe concept names, and does NOT touch the database — all of that is deterministic code in lib/actions/explainBack.ts running after the call returns.",
    },
    trace: {
      prompt:
