    ...
    sections: [
      {
        ...
        body:
          ...
          "A Concept is the thing actually being learned — \"Mixture of Experts,\" say — identified by a unique slug so the same idea encountered in two different sessions resolves to one row, not two. It also carries a field, a broad topic area (\"Machine learning\") the model assigns the first time it names the concept and which is kept for life after that; the Mindscape uses it to group concepts into colonies, trees, or star regions. Sessions and Concepts are linked through sessionConcepts, a join table with a role (\"primary\" vs \"mentioned\"). Notably, Concept has no stored confidence score or mastery percentage — deliberately. Understanding is derived at read time from the explain-back and recall history attached to it (deriveStanding in lib/knowledge.ts, which centralizes what used to be spread across a few functions in lib/queries.ts), not cached as a number that could silently drift out of sync with the evidence behind it.",
      },
      {
